...
public class Bullet : Projectile
{
    ...
    public override void OnCollisionEnter2D(Collision2D other)
    {
        ...
        GameObject lol;
        if (HitEffect1) lol = Instantiate(HitEffect1, transform.position, transform.rotation);
        if(HitEffect2) lol = Instantiate(HitEffect2, transform.position, transform.rotation);
